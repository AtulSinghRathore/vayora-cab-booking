...
export default function FindBookingPage() {
  ...
  async function update(action: "cancel" | "amend", form?: HTMLFormElement) {
    ...
    const result = await response.json(); setMessage(result.message || result.error); setMessageTone(response.ok ? "success" : "error"); if (response.ok) { setAmending(false); setBooking((current) => current ? { ...current, status: action === "cancel" ? "cancelled" : "amendment_requested" } : current); }
  }
