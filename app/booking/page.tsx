...
export default function FindBookingPage() {
  ...
  async function find(event: FormEvent) {
    ...
    if (response.ok) setBooking(result); else setMessage(result.error || "Booking not found.");
  }
  async function update(action: "cancel" | "amend", form?: HTMLFormElement) {
    ...
    const result = await response.json(); setMessage(result.message || result.error); if (response.ok) { setAmending(false); setBooking((current) => current ? { ...current, status: action === "cancel" ? "cancelled" : "amendment_requested" } : current); }
  }

  return <main className="portal-page">
    <header className="site-header"><Link className="brand" href="/"><span className="brand-mark">V</span><span>Vayora</span></Link><nav className="desktop-nav"><Link href="/#book">Outstation</Link><Link href="/airport">Airport</Link><Link className="active" href="/booking">Find booking</Link></nav><a className="phone-link" href="tel:+919304591415">+91 93045 91415</a></header>
    <section className="portal-shell">
      ...
      <p className="booking-status" role="status">{message}</p>
      {booking && <article className="booking-record"><div className="record-heading"><div><p>{String(booking.id)}</p><h2>{String(booking.pickup)} → {String(booking.destination)}</h2></div><span className={`status-pill status-${booking.status}`}>{String(booking.status).replaceAll("_", " ")}</span></div><dl>{[["Travel date",booking.travel_date],["Pickup time",booking.pickup_time],["Vehicle",booking.vehicle],["Estimated fare",`₹${Number(booking.fare_total).toLocaleString("en-IN")}`],["Driver",booking.driver_name || "Assigned after approval"],["Driver phone",booking.driver_phone || "Shared after assignment"]].map(([k,v])=><div key={String(k)}><dt>{k}</dt><dd>{v}</dd></div>)}</dl>{!["cancelled","rejected","completed"].includes(String(booking.status)) && <div className="record-actions"><button type="button" onClick={() => setAmending(!amending)}>Amend booking</button><button className="danger-button" type="button" onClick={() => update("cancel")}>Cancel booking</button></div>}{amending && <form className="amend-form" onSubmit={(e) => { e.preventDefault(); update("amend", e.currentTarget); }}><label><span>New travel date</span><input name="travelDate" type="date" defaultValue={String(booking.travel_date)} required /></label><label><span>New pickup time</span><input name="pickupTime" type="time" defaultValue={String(booking.pickup_time || "08:00")} required /></label><label className="full-field"><span>What should be changed?</span><textarea name="note" rows={3} /></label><p>Date/time/address amendments are reviewed by Vayora. Ordinary amendments are free; a travel-date change may carry a separately confirmed fee.</p><button className="primary-button">Submit amendment</button></form>}</article>}
    </section>
  </main>;
}
